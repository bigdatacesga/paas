...
http POST http://paas:6000/bigdata/api/v1/products name=cassandra version=3.0.8 description='The Cassandra database is the right choice when you need scalability and high availability without compromising performance.' logo_url='http://hadoop.cesga.es/img/1280px-Cassandra_logo.svg.png' x-auth-token:$TOKEN
