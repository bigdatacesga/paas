...
def create_service():
    ...
    registry.connect(ENDPOINT)
    template = registry.register(name=template_data["name"], version=template_data["version"], description="",
                      template="", options="")
...
@api.route('/services/<service>/<version>', methods=['PUT'])
@api.route('/services/<service>/<version>/', methods=['PUT'])
def fill_service(service, version):
    # Get data from user parameters
    template_data = ut.parse_put_template_data(request)

    template = registry.register(name=service, version=version, description=template_data["description"],
                      template=template_data["template"], options=template_data["options"])
    return str(template), 200
